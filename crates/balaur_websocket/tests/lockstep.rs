...
/// Keep both peers ticking until every tick through `through` has had both
/// players' inputs arrive on both sides.
///
/// Waited on rather than slept for: loopback has no latency guarantee, and a
/// pause long enough on one machine is short on another — which is how this
/// suite came to pass on Linux and fail on Windows.
fn settle(pair: &mut Pair, through: u64) {
    let confirmed = |pair: &Pair| {
        (1..=through)
            .all(|at| pair.host.session().confirmed(at) && pair.guest.session().confirmed(at))
    };
    for _ in 0..200 {
        if confirmed(pair) {
            return;
        }
        let tick = pair.host.tick();
        step(pair, tick);
    }
    assert!(confirmed(pair), "the peers never settled tick {through}");
}
