...
pub const ENGINE_OPS: &[EngineOp] = &[
    ...
    EngineOp {
        module: "scene",
        name: "set_variable",
        call: scene_set_variable,
    },
    EngineOp {
        module: "scene",
        name: "variables",
        call: scene_variables,
    },
    EngineOp {
        module: "engine",
        name: "timings",
...
fn scene_switch(eng: &Engine, args: &[Value]) -> Result<Value> {
...
fn scene_variable(eng: &Engine, args: &[Value]) -> Result<Value> {
...
fn scene_set_variable(eng: &Engine, args: &[Value]) -> Result<Value> {
    let name = text(args, 0)?;
...
fn scene_variables(eng: &Engine, _args: &[Value]) -> Result<Value> {
    let variables = eng.resource::<crate::variables::Variables>();
    let variables = variables.borrow();
    Ok(Value::List(
        variables
            ...
            .map(|name| {
                ...
                Value::Map(vec![
                    ...
                    (
                        "type".into(),
                        Value::Str(
                            spec.map_or("float", |s| s.kind.word()).to_string(),
                        ),
                    ),
                    (
                        "value".into(),
                        spec.map_or(Value::Nil, |s| s.value.clone()),
                    ),
                    (
                        "persist".into(),
