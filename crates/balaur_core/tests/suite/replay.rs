...
fn a_recording_round_trips_through_the_file() {
    ...
    let mut recorder = Recorder::create(balaur_core::files::default_backend(), &path, header(), true, 0).unwrap();
    for tick in 1..=3u64 {
        recorder
            .write(&Frame {
                tick,
...
fn a_recording_from_a_future_format_is_refused_by_name() {
    ...
    let mut recorder = Recorder::create(balaur_core::files::default_backend(), &path, future, false, 0).unwrap();
...
fn a_session_records_into_the_backend_rather_than_the_disk() {
    std::thread::spawn(|| {
        ...
        let mut recorder =
            Recorder::create(fs.clone(), path, header(), false, 0).unwrap();
        recorder
            .write(&Frame {
                tick: 1,
                dt: (1.0f32 / 60.0).to_bits(),
