...
fn a_wide_anchor_spans_one_axis_and_states_or_measures_the_other() {
    ...
    let bar = add_widget(
        &app,
        &toml::toml! { kind = "panel" text = "top" anchor = "fill_top" height = 40.0 y = 0.0 }.into(),
    );
    let side = add_widget(
        &app,
    ...
    let band = add_widget(
        &app,
        &toml::toml! { kind = "panel" text = "band" anchor = "fill_across" height = 60.0 y = 0.0 }.into(),
    );
    let ctx = egui::Context::default();
    settle(&app, &ctx);

    let top = rect(&ctx, bar);
    assert!((top.width() - 640.0).abs() < 1.0, "fill_top spans the width: {top:?}");
    assert!((top.height() - 40.0).abs() < 1.0, "and keeps the height it states: {top:?}");
    assert!(top.min.y.abs() < 1.0, "along the top edge: {top:?}");

    let right = rect(&ctx, side);
    assert!((right.height() - 480.0).abs() < 1.0, "fill_right spans the height: {right:?}");
    assert!(
        right.width() > 1.0 && right.width() < 320.0,
        "and measures its width, stating none: {right:?}"
    );
    assert!((right.max.x - 640.0).abs() < 1.0, "against the right edge: {right:?}");

    let middle = rect(&ctx, band);
    assert!((middle.width() - 640.0).abs() < 1.0, "fill_across spans the width: {middle:?}");
    assert!((middle.center().y - 240.0).abs() < 1.0, "through the middle: {middle:?}");
}
