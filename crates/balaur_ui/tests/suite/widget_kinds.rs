...
/// A menu's rows can be nodes. A flat list of strings carries no icon, no
/// shortcut and no tick, which is every row the shell's own menu draws.
#[test]
fn a_menu_draws_its_children_as_rows() {
    let (_dir, app) = app();
    let params = toml::toml! { kind = "menu" text = "Menu" x = 10.0 y = 10.0 };
    let host = add_widget(&app, &params.into());
    for (name, caption) in [("R0", "Command palette"), ("R1", "Settings")] {
        let row = toml::toml! { kind = "button" text = (caption) width = 173.0 height = 22.0 };
        add_child_widget(&app, host, name, &row.into());
    }
    let ctx = egui::Context::default();
    settle(&app, &ctx);
    // The captions shape through the text renderer rather than an egui
    // galley, so the rows are counted by the boxes they took.
    let rows = |out: &egui::FullOutput| {
        out.shapes
            .iter()
    ...
    let shut: Vec<_> = pass(&app, &ctx, vec![])
        .shapes
        .iter()
        ...
        .collect();
    panic!("closed pass rects: {shut:#?}");
    let at = root_rect(&ctx, host).center();
    pass(&app, &ctx, press(at, true));
    pass(&app, &ctx, press(at, false));
    assert_eq!(
        rows(&pass(&app, &ctx, vec![])),
        2,
        "the open menu drew no rows of its own"
    );
}
