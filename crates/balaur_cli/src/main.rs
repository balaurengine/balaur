...
fn main() -> Result<()> {
    ...
    match Cli::parse_from(argv()).command {
        ...
        Command::New { path, template } => new_project(&path, template.as_deref()),
        Command::Run {
            path,
            headless,
            frames,
...
fn new_project(path: &Path, template: Option<&str>) -> Result<()> {
    let name = path
        .file_name()
    ...
    if let Some(template) = template {
        ...
        if !from.is_dir() {
            let mut known: Vec<String> = std::fs::read_dir(library.join("templates"))?
                ...
                .collect();
            known.sort();
            anyhow::bail!("no template `{template}`; the templates are {}", known.join(", "));
        }
        copy_template(&from, path, &name)?;
        tracing::info!("created project '{name}' from the `{template}` template at {}", path.display());
