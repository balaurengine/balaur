...
fn main() -> Result<()> {
    ...
    match Cli::parse().command {
        ...
        Command::Import {
            ...
            project,
            layers,
        } => import_file(&file, &project, &layers),
...
fn import_file(file: &Path, project: &Path, layers: &[String]) -> Result<()> {
    let extension = file
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or_default()
...
fn import_sprite(file: &Path, project: &Path, layers: &[String]) -> Result<()> {
    ...
    let mut written = vec![(texture, imported.png), (format!("sheets/{stem}.toml"), imported.sheet.into_bytes())];
...
#[cfg(test)]
mod tests {
    use super::{import_file, joinable, run_test, test_scripts};
    use std::path::{Path, PathBuf};
    ...
    fn importing_a_sprite_writes_a_page_a_sheet_and_a_clip_per_tag() {
        ...
        let sheet: toml::Value =
            toml::from_str(&std::fs::read_to_string(project.path().join("sheets/walk.toml")).unwrap())
                .unwrap();
