...
//!
//! The final web binary must link with `-sFETCH -lwebsocket.js` — cargo
//! cannot propagate link args from a library, so the game's own build (or
//! `.cargo/config.toml` for the emscripten target) carries them.
