...
fn clip(x: f64, extra: &str) -> toml::Value {
    toml::from_str(&format!(
        ...
        property = "position"
        interp = "linear"
        keys = [{{ t = 0.0, value = [{x}, 0.0, 0.0] }}, {{ t = 1.0, value = [{x}, 0.0, 0.0] }}]
        {extra}
        "#
    ))
    .unwrap()
...
const GROW: &str = r#"
        ...
        property = "scale"
        interp = "linear"
        keys = [{ t = 0.0, value = [1.0, 1.0, 1.0] }, { t = 1.0, value = [3.0, 3.0, 3.0] }]
"#;
...
fn hero(app: &App) -> Entity {
    ...
    balaur_anim::define(eng, entity, "left", clip(0.0, GROW)).unwrap();
    balaur_anim::define(eng, entity, "right", clip(1.0, "")).unwrap();
    balaur_anim::define(eng, entity, "far", clip(2.0, "")).unwrap();
    entity
}
...
fn a_track_only_the_outgoing_clip_keys_keeps_playing_through_the_fade() {
    ...
    tick(&mut app, 15);
    balaur_anim::player::play_faded(&app.engine, hero, "right", 1.0, Easing::LINEAR, true).unwrap();
    tick(&mut app, 30);
...
fn a_fade_started_mid_fade_carries_on_from_where_the_blend_was() {
    ...
    balaur_anim::play(&app.engine, hero, "left").unwrap();
    tick(&mut app, 5);
    balaur_anim::player::play_faded(&app.engine, hero, "right", 0.5, Easing::LINEAR, true).unwrap();
    tick(&mut app, 15);
    let before = transform(&app, hero).position.x;
    balaur_anim::player::play_faded(&app.engine, hero, "far", 0.5, Easing::LINEAR, true).unwrap();
    tick(&mut app, 1);
...
fn a_clip_that_ends_mid_fade_leaves_nothing_to_blend_later() {
    ...
    let once: toml::Value = toml::from_str(
        ...
        [[tracks]]
        property = "position"
        keys = [{ t = 0.0, value = [3.0, 0.0, 0.0] }, { t = 0.25, value = [3.0, 0.0, 0.0] }]
        "#,
    )
    .unwrap();
    balaur_anim::define(&app.engine, hero, "once", once).unwrap();
    balaur_anim::play(&app.engine, hero, "left").unwrap();
    balaur_anim::player::play_faded(&app.engine, hero, "once", 1.0, Easing::LINEAR, true).unwrap();
    tick(&mut app, 30);
    assert_eq!(
        balaur_anim::current(&app.engine, hero),
        None,
        "`once` ended"
    );
    assert_eq!(fades(&app, hero), 0);
