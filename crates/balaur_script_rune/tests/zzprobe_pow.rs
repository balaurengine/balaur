...
fn eval(src: &str) -> f64 {
    ...
    let unit = rune::prepare(&mut sources).with_context(&ctx).with_diagnostics(&mut d).build().unwrap();
...
fn rune_pow_matches_libm_bit_for_bit() {
    for (x, y) in [(2.0f64, 0.5f64), (1.7, 3.3), (10.0, -2.25), (0.3, 7.0)] {
        ...
        println!("PROBE powf({x}, {y}) rune={got:e} libm={:e}", libm::pow(x, y));
    ...
    for (x, n) in [(2.0f64, 3i32), (1.7, 5), (0.3, -2)] {
        ...
        println!("PROBE powi({x}, {n}) rune={got:e} libm={:e}", libm::pow(x, f64::from(n)));
        assert_eq!(got.to_bits(), libm::pow(x, f64::from(n)).to_bits(), "powi({x}, {n})");
    }
}
