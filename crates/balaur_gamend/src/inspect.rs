...
pub(crate) fn install(m: &mut dyn Bindings<Engine>) {
    m.describe(&[
        ...
        ("server", &[], "()", "The server a `configure()` with no URL uses: `{ name, url, production, local, plugin }` from `[gamend]`, `name` being `production` or `local`."),
