...
fn mesh_of(eng: &Engine, value: Option<&Value>) -> Result<(Vec<Vec3>, Vec<[u32; 3]>)> {
    match value {
...
fn mesh_value(points: &[Vec3], indices: &[[u32; 3]]) -> Value {
    map([
        (
            "points",
            Value::List(
...
pub(crate) fn install_geometry_api(m: &mut dyn Bindings<Engine>) {
    ...
    m.function("convex_hull", |eng: &Engine, mesh: Value| {
        ...
        let (hull_points, hull_indices) = rapier3d::parry::transformation::try_convex_hull(&points)
    ...
    m.function(
        ...
        |eng: &Engine, (mesh, opts): (Value, Option<Value>)| {
            ...
            let mut params = rapier3d::parry::transformation::vhacd::VHACDParameters::default();
            params.resolution = opts.f32("resolution", params.resolution as f32).max(1.0) as u32;
            params.concavity = opts.f32("concavity", params.concavity);
            params.max_convex_hulls = opts
                .f32("max_pieces", params.max_convex_hulls as f32)
                .max(1.0) as u32;
            let vhacd = rapier3d::parry::transformation::vhacd::VHACD::decompose(
    ...
    m.function(
        ...
        |eng: &Engine, (mesh, opts): (Value, Option<Value>)| {
            ...
            let volume = rapier3d::parry::transformation::voxelization::VoxelizedVolume::voxelize(
            ...
            for i in 0..rx {
                for j in 0..ry {
                    for k in 0..rz {
                        let filled = matches!(
                            volume.voxel(i, j, k),
                            rapier3d::parry::transformation::voxelization::VoxelValue::PrimitiveInsideSurface
                                | rapier3d::parry::transformation::voxelization::VoxelValue::PrimitiveOnSurface
                        );
                        if filled {
            ...
            Ok(map([
                ("size", Value::Vec3([size, size, size])),
    ...
    m.function("split", |eng: &Engine, (mesh, opts): (Value, Value)| {
        ...
        let normal = Vec3::from(opts.vec3("normal", [0.0, 1.0, 0.0]));
        ...
        let result = trimesh.split(&Pose3::IDENTITY, normal, normal.dot(point), 1.0e-5);
        ...
        Ok(match result {
            rapier3d::parry::query::SplitResult::Pair(a, b) => {
            ...
            rapier3d::parry::query::SplitResult::Negative => {
                Value::List(vec![half(Some(&trimesh)), Value::Nil])
            }
            rapier3d::parry::query::SplitResult::Positive => {
    ...
    m.function("intersect", |eng: &Engine, (a, b): (Value, Value)| {
        ...
        let found = rapier3d::parry::transformation::intersect_meshes(
            &Pose3::IDENTITY,
            &first,
            false,
            &Pose3::IDENTITY,
