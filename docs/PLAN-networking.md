...
> (`multiplayer/faults`) and applies to every session link. `ARCHITECTURE.md`
...
> `docs/PLAN-multiplayer.md` is the engine half (a match a script can open
> through the `multiplayer` module, `peer` / `host` / `server` topologies,
> server-ordered inputs, late join, reconnect, spectators, host migration,
> bincode on the wire, the editor's Multiplayer dock) and
> `docs/PLAN-gamend.md` is the server half (a game server per lobby, lobby
> tokens, rejoin, the match record, typed bindings). Voice got
> `docs/PLAN-voice.md`. §0 and §1 below gained the rows that came out of
> it.
>
...
- The roster is fixed at construction and every peer is the same peer: no
  ...
  and nothing joins, leaves, reconnects or migrates. `docs/PLAN-multiplayer.md`
  steps 2 to 6.
...
| Bots | Not engine code: a script writes a slot through `multiplayer::set_input_for` (`docs/PLAN-multiplayer.md` §2) |
...
8. **Matches from Gamend.** Split on 2026-09-05: the engine half is
   `docs/PLAN-multiplayer.md` (a match a script can open through the
   `multiplayer` module, roles, late join, reconnect, migration) and the
   server half is `docs/PLAN-gamend.md` (a game server per lobby, lobby
   tokens, rejoin, the match record, typed bindings). Done when
   `docs/PLAN-gamend.md` step E2 is: two engines matched by Gamend play on a
   server Gamend launched. First point at which a real game ships on this
   stack, which is why it comes before replication rather than after.
