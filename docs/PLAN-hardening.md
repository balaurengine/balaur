...
click changes the digest through state no recording carries; a freed node
leaves its
...
| --- | --- | --- |
| `crates/balaur_ui/src/widget/layer.rs`, `crates/balaur_core/src/digest.rs:219-233` — *verified* | `settle_clicks` and `settle_edits` write `clicked`, `active`, `width` and `height` onto the `Widget` component from egui's own event pump, after the tick; `push_components` hashes every property a component's `get` reports, `readonly` included. A click, a seam drag or a tab change moves the digest of a tick no recording can reproduce, and `on_click` handlers never run headless | skip `readonly` properties in `push_components`; then route clicks, focus and seams through `InputSnapshot` so the handlers run on replay; a `replay --verify` test with one clicked button |
| ~~`crates/balaur_animation`~~ — **built**, `crates/balaur_animation/src/snapshot.rs` | was: `Playback.time`, `playing`, the queue, every tween and `AnimationState.accumulator` survived a rollback untouched, so a re-simulated kinematic platform pushed bodies from the wrong pose | a `snapshot` and a `digest` source keyed by `StableId`, a replay-setup hook for the residual, and `tests/rollback.rs`. Every jiggle spring rides them too |
...
| ~~`crates/balaur_animation/src/modifier.rs`~~ — **built**, `MIN_BONE` | was: `two_bone_ik` clamped `d` to `(|l1-l2| + 1e-5, l1 + l2 - 1e-5)`, which is `min > max` for a bone under `1e-5` and passed the `1e-6` guard; `f32::clamp` panics | rejected by `min(l1, l2) <= MIN_BONE`, with `a_bone_too_short_for_the_solver_leaves_the_pose_alone` |
...
| `crates/balaur_input/Cargo.toml:3`, `balaur_http/src/lib.rs:12`, `balaur_gamend/src/lib.rs:12`, `balaur_ui/src/immediate/mod.rs`, `docs/NAMING.md:312, 408`, `docs/generated/behaviour.md:665` | Luau left on 2026-09-02; these still say Lua (the input crate's description reaches the website's Crates page) | delete |
...
| `editor/scripts/docks.rn:15-24`, `plugins.rn:116`, `editor/plugins/counter.rn:74` — *verified* | Plugin docks are unreachable: the tab lists are fixed, `registry.docks` is consulted for names only, nothing pushes a plugin id, and `counter.rn` writes `S.dock`, a field that is gone; EDITOR-SCREENS §5 promises one tab per plugin | push each registered dock into `S.docks.bottom.tabs`; assert it in `test:plugin_docks` |
...
| `editor/scripts/settings.rn:20-23, 161, 175-183`, `shell.rn:348` | `settings_search:<query>` sets `S.settings.query` but not the search field's buffer, which the next frame writes back as `""`; a category click while a query is typed has the same shape | write both, clear both |
| `editor/scripts/layout.rn:146-160`, `center.rn:39, 79, 251`, `chrome.rn:41` | The widget layer draws after `draw_ui`, so rects read back are a frame behind (`ui.widget_rect` is in fact two: `roll_measurements` runs at the start of `widget_layer::draw`, `widget_arrange.rs:55-68`); only `docks::draw` and `status_row` skip a zero rect, so the rail, the tabs, the hooks sheet, the workspace bar and the viewport chrome draw at `0,0,0,0` on the first frame and after any size change — D18 again, one frame at a time | roll at the end of `draw`; one zero-rect guard in `style::sheet`; `test:layout` asserts no published rect is zero |
...
| `scripts/e2e.sh` | `test:lint` is never run, `breakpoint` and `pose` assert nothing, `rotdemo` is referenced nowhere; `test:layout` never runs at `scale:2.4` | add them |
| per frame | Up to three `fs::exists` for the brand logo (`chrome.rn:16-23`); `docks::panels` rebuilt about eleven times a frame, re-splitting every script's source in the Script workspace; `log::recent(500)` lowercased per frame; the palette list and the settings sidebar rebuilt per frame while open | three small caches |
...
- ARCHITECTURE.md: `NODE_OPS` is 31 operations, not 28; the replay sources
  ...
  "five workspaces, fixed five-region layout" and "one node" predate Stage and
  the shell tree; `camera.post` and the settings subsystem have no section
  at all.
...
- The animation snapshot and digest sources are built
  (`balaur_animation::snapshot`); **PLAN-input.md** takes the Bluetooth feature report, the twin
