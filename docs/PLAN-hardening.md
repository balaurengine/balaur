...
| ~~`crates/balaur_anim/src/modifier.rs`~~ — **built**, `MIN_BONE` | was: `two_bone_ik` clamped `d` to `(|l1-l2| + 1e-5, l1 + l2 - 1e-5)`, which is `min > max` for a bone under `1e-5` and passed the `1e-6` guard; `f32::clamp` panics | rejected by `min(l1, l2) <= MIN_BONE`, with `a_bone_too_short_for_the_solver_leaves_the_pose_alone` |
