...
| D21 | **The shell's first frame is a corner pile-up.** Rects read back through `ui.widget_rect` are two frames behind, and only the dock and the status row skip a zero rect; the rail, the tabs, the hooks sheet, the persona bar and the viewport chrome draw at `0,0,0,0` on the first frame and after any size change. | `layout.rn`, `center.rn`, `chrome.rn` | `--frames 1` |
