...
# Lines of drift printed per stale file; past that a log is scrolled, not read.
MAX_DRIFT = 60
...
def drift(name, have, want):
    ...
    rest = len(diff) - MAX_DRIFT
    return "\n".join(diff[:MAX_DRIFT] + ([f"... {rest} more lines"] if rest > 0 else []))
...
def main():
    ...
    if args.check:
        if stale:
            ...
            for name, (have, want) in sorted(stale.items()):
                print(drift(name, have, want), file=sys.stderr)
